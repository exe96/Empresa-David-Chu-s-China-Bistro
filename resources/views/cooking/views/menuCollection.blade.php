  ...
  <div class="row gy-3 gx-3">
    @foreach ($menu as $item )
      <div class="col-xl-3 col-md-4 col-sm-6 col-12 item-menu-content">  
        ...
        @endauth
        <a class="item-menu" href="http://192.168.1.7:8080/home/cooking/menu/{{$item->letter}}">
          <div class="category-tile">
          
            
              {{-- cambiar --}}
            <img class="photo-item-menu" width="200" height="200" onerror="this.onerror=null;this.src='/images/restaurant/menu/default/category.jpg';" 
            @if(!($item->extension=='4'))
            @if($item->extension=="2")
            src= " /images/restaurant/menu/{{$item->letter}}/{{$item->letter}}.jpg"
            @elseif($item->extension=="1")
            src= " /images/restaurant/menu/{{$item->letter}}/{{$item->letter}}.jpeg"
            @elseif($item->extension=="3")
            src= " /images/restaurant/menu/{{$item->letter}}/{{$item->letter}}.png"
            @endif
            @else
            src='/images/restaurant/menu/default/category.jpg'
            @endif  
